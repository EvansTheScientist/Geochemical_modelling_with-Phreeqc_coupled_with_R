...
init <- c(Al=NA, C= 9.837e-04, Ca=1.289e-03, Cl=1.134e-03, Fe=NA,
          K=1.337e-04, Mg=9.269e-05, Na=1.451e-04, S=1.042e-03, Si=NA,
          pH=3.809)
...
## modified phases
primary <- c("Plagioclase","Albite", "Anorthite", "K-mica", "K-feldspar", "Quartz", 
             "Chalcedony", "Pyrite", "Fe(OH)3(a)")
secondary <- c("Kaolinite", "Illite", "Chlorite(14A)", "Goethite", "Calcite")
...
out <- barplot(cviz2, beside=TRUE, log="y", ylim = c(1E-9,10),
               col=c("yellow", "light green", "grey"), las=1)
...
library(readxl)
samples <- read_excel("Pra_data_M.xlsx", sheet=2)
