...
## from median "Evaporated_Rainwater_Conc.txt which was achieved by 47.2% evaporation"
ewat <- c("SOLUTION 1",
          "units mol/kgw",
          "temp 25",
          "pressure 1",
...
init <- c(Al=NA, C=2.649e-03, Ca=3.472e-03, Cl=3.054e-03, Fe=NA,
          K=3.601e-04, Mg=2.496e-04, Na=3.906e-04, S=2.807e-03, Si=NA,
          pH=3.426)
...
## modified phases, phlogopite used as biotitie and k mica as muscovite
primary <- c("Albite", "K-mica", "Phlogopite", "K-feldspar", "Quartz",
             "Chalcedony", "Pyrite")
secondary <- c("Kaolinite", "Illite", "Calcite", 
              "Ca-Montmorillonite", "Goethite","Fe(OH)3(a)", "Chlorite(14A)")
...
################################################################################################################
## If we want to save the outputs then we can activate the commented codes below
saveRDS(file="20220301_Southern_evolution_optimization_Final_Mean.rds", object=list(dat=dat, tot=tot, primary=primary, secondary=secondary, filtered=filtered))
...
calib <- optimise(f=ToOptimizePE, input=inps[[14]], target = cvec,
...
temp <- Distribute(inps[[14]], prop="pe", values=calib$minimum) %>%
...
out <- barplot(cviz, beside=TRUE, log="y", ylim = c(1E-9,10),
               col=c("yellow", "light green", "grey"), las=1)
legend("topleft", c("Calibrated", "Final", "target"),
       fill=c("yellow", "light green", "grey"), bty="n", cex=0.8)
mtext(side=2, line=3.2, "mol/kgw", font=1)
#######################################
...
out <- barplot(table, ylab="Frequency", font=1, las=2, col=c("cyan"))
...
mtext(side=2, line=2.2, "mol/kgw", font=1)
...
barplot(textCa$delta, names.arg=row.names(textCa), las=2, cex.axis=1, cex=1)
