# Time-stamp: "Last modified 2023-02-03 21:13:35 delucia"
...
## from median "Evaporated_Rainwater_Conc.txt" Rainwater to northern zone: 47% evaporation
ewat <- c("SOLUTION 1",
          "units mol/kgw",
          "temp 25",
          "pressure 1",
...
init <- c(C=2.065e-04, Ca=2.707e-04, Cl=2.381e-04, Fe=NA,
          K=2.807e-05, Mg=1.946e-05, Na=3.045e-05, S=2.188e-04, Si=NA,
          pH=4.431)
...
## saveRDS(file="20230203_Northern_evolution_optimization_SolH.rds", object=list(dat=dat, tot=tot, primary=primary, secondary=secondary, filtered=filtered))
## aa <- readRDS(file="20230203_Northern_evolution_optimization_SolH.rds")
## dat <- aa$dat
## tot <- aa$tot
## primary <- aa$primary
## secondary <- aa$secondary
## filtered <- aa$filtered


ind_all <- ComputeMetric(dat, target = cvec, FUN = "rrmse")

colnames(dat)
restot <- as.data.frame(lapply(colnames(dat),
                               function(x) ComputeMetric(dat, target = cvec, FUN = "rrmse", comp=x)),USE.NAMES=FALSE)
...
library(readxl)
samples <- read_excel("Pra_data_M_Good.xlsx", sheet=1)
...
#which(rowSums(inrange_noph)==ncol(ranges[, -c(4,5)])) ## 229
